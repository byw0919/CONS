...
class Agents:
    def __init__(self, args):
        ...
        if args.alg == 'vdn':
            ...
            from policy.cons import CONS
            self.policy = CONS(args)
    ...
    def choose_action(self, obs, last_action, agent_num, avail_actions, epsilon, episode_counts=None, agent_obs=None, epi_obs=None, ask_budget=None, maven_z=None, evaluate=False):
        inputs = obs.copy()
        avail_actions_ind = np.nonzero(avail_actions)[0]  # [0 1 2 3 4](ndarray)  # index of actions which can be choose
        ...
        elif self.args.alg.find('cons') > -1:
            ...
            if np.random.uniform() >= epsilon:
                ...
                if avail_actions_ind.shape[0] <= 1:
                    ...
                    if episode_counts > self.args.start_advice and ask_budget[agent_num] > 0:  # able to send a request
                        action, ask_budget = self.policy.ask_advice(q_value, hidden_state,
                        ...
                        if np.random.uniform() < epsilon:
                            ...
                            action = torch.argmax(q_value)
        elif self.args.alg.find('adhoc_td') > -1:
            q_value[avail_actions == 0.0] = - float("inf")
            if np.random.uniform() >= epsilon:
                action = torch.argmax(q_value)
            else:
                if avail_actions_ind.shape[0] <= 1:
                    ...
                    if episode_counts > self.args.start_advice and ask_budget[agent_num] > 0:  # able to send a request
                        action, ask_budget = self.policy.ask_advice(q_value, hidden_state, obs, last_action, agent_num,
                                                                      agent_obs, epi_obs, ask_budget, episode_counts)
                        if (action is None) or (action not in avail_actions_ind):  # knowledge is not available
                            # perform epsilon-greedy
                            if np.random.uniform() < epsilon:
                                action = np.random.choice(avail_actions_ind)
    ...
    def train(self, batch, train_step, episode_counts, epsilon=None):  # coma needs epsilon for training
...
class CommAgents:
    ...
    def train(self, batch, train_step, episode_counts, epsilon=None):  # coma在训练时也需要epsilon计算动作的执行概率
